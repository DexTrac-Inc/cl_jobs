...
- Docker support for easier deployment and management
...
- `config/`: Configuration files (mounted as read-only in containers)
...
With Docker:
```bash
docker compose up -d slack_manager
```
...
In Docker mode, logs are written to the mounted logs directory.
